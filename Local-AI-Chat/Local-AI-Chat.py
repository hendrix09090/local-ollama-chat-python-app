...
class ChatApp:
    def __init__(self, page: ft.Page):
        ...
        self.stop_ai_flag = False  # Flag to stop AI response
        self.stop_button = ft.ElevatedButton(
            "Stop AI",
            on_click=self.handle_stop_ai,
            disabled=True,
            color=ft.Colors.RED
        )
        ...
        message_container = ft.Row(
            controls=[
                ...
                ft.IconButton(
                    ...
                    tooltip="Copy message"  # Add tooltip for better UX
                )
            ],
            ...
            try:
                ...
                try:
                    for line in response.iter_lines():
                        if self.stop_ai_flag:  # Check if stop was requested
                            async def handle_stop():
                                cleanup()
                                self.show_error("AI response stopped", ft.Colors.ORANGE)
                            self.page.run_task(handle_stop)
                            return
    ...
    def handle_stop_ai(self, e):
        """Handle stopping the AI response"""
        self.stop_ai_flag = True
        self.stop_button.disabled = True
        self.page.update()

    def reset_ai_state(self):
        """Reset AI state after stopping"""
